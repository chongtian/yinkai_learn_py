...
def make_pipeline(context):
    ...
    # Define factors.
    ma_fast = SimpleMovingAverage(
        inputs=[EquityPricing.close], window_length=fast_ma_periods)
    ma_slow = EWMA.from_span(
        inputs=[EquityPricing.close],  window_length=slow_ma_periods, span=15)
...
def rebalance(context, data):
    ...
    if buy and p.amount == 0 or context.hold_days == -1:
        ...
        algo.order_optimal_portfolio(objective, constraints)
        log.info('Buy {0} after {1} periods.'.format(
            context.assets[0], context.hold_days))
        context.hold_days = 0
    ...
    if sell and p.amount > 0 and context.hold_days > context.threshold_hold_days and (return_percent < context.threshold_sell_loss or return_percent > context.threshold_sell_win):
        ...
        if context.move_fund_out_of_market:
            ...
            log.info('Sell {0} after {1} periods.{2:%}'.format(
                context.assets[0], context.hold_days, return_percent))
        algo.order_optimal_portfolio(objective, constraints)
        context.hold_days = 0
